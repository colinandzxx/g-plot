...
__device__ static void shabal_core(void *cc, const unsigned char *data,
                                   size_t len)
{
  sph_shabal_context *sc;
  unsigned char *buf;
...
__device__ static void shabal_close(void *cc, unsigned ub, unsigned n,
                                    void *dst, unsigned size_words)
{
  sph_shabal_context *sc;
  unsigned char *buf;
  size_t ptr;
...
/* see sph_shabal.h */
__device__ void sph_shabal192_addbits_and_close(void *cc, unsigned ub,
                                                unsigned n, void *dst)
{
...
/* see sph_shabal.h */
__device__ void sph_shabal224_addbits_and_close(void *cc, unsigned ub,
                                                unsigned n, void *dst)
{
...
/* see sph_shabal.h */
__device__ void sph_shabal256_addbits_and_close(void *cc, unsigned ub,
                                                unsigned n, void *dst)
{
...
/* see sph_shabal.h */
__device__ void sph_shabal384_addbits_and_close(void *cc, unsigned ub,
                                                unsigned n, void *dst)
{
...
/* see sph_shabal.h */
__device__ void sph_shabal512_addbits_and_close(void *cc, unsigned ub,
                                                unsigned n, void *dst)
{
...
__global__ void sha(uint8_t *g_out)
{
  ...
  for (int i = 0; i < SCOOPS_PER_NONCE * 2; i++)
  {
    ...
    char dst[32] = {};
    sph_shabal256_close(&cc, &g_out[gid * bid + i * HASH_SIZE]);
...
struct plotter_ctx
{
  ...
  plotter_ctx(const uint32_t grids)
      : grids(grids)
  {
  ...
  int plot(uint64_t addr, uint64_t start_nr)
  {
    ...
    checkCudaErrors(cudaMemcpy(sols, &g_sols, PLOT_SIZE * THREADS_PER_BLOCK * grids,
